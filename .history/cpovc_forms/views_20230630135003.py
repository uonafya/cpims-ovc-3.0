    ...
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                ...
                benchmark_data = OVCBenchmarkMonitoring.objects.filter(is_void=False, household=house_hold, event_date__range=[today_less90, today])   #filter(event=event).order_by('event_date'))
            except Exception as e:
                benchmark_data = {}
                print(e)
...
def edit_grad_monitor(request, id):
    ...
    caregiver_id = OVCRegistration.objects.get(person=child).caretaker_id
    caregiver = RegPerson.objects.get(id=caregiver_id)
    form_typ = OVCBenchmarkMonitoring.objects.filter(obm_id=id,is_void=False).values_list()[0][3]
