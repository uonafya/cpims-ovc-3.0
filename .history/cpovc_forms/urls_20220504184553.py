...
urlpatterns = [
    ...
    # OVC Care - Form1A
    re_path('form1a/new/(?P<id>\d+)/',
            views.form1a_events, name='form1a_events'),
    ...
    re_path('cpara/delete/(?P<id>\d+)/(?P<btn_event_pk>.+)/',
            views.delete_cpara, name='delete_cpara'),

    # OVC Care - Case Plan Template
    re_path('caseplan/new/(?P<id>\d+)/',
            views.case_plan_template, name='new_caseplan'),
    re_path('caseplan/update/(?P<ovcid>\d+)/(?P<event_id>.+)/',
            views.update_caseplan, name='update_caseplan'),
    re_path('caseplan-monitoring/new/(?P<id>\d+)/',
            views.new_case_plan_monitoring, name='new_caseplan_monitoring'),

    # wellbeing Adult and Child
    re_path(r'^wellbeing/new/(?P<id>\d+)/$',
            views.new_wellbeing, name='new_wellbeing'),

    # wellbeing Adolescent
    re_path(r'^wellbeingadolescent/new/(?P<id>\d+)/$',
            views.new_wellbeingadolescent, name='new_wellbeingadolescent'),
    # hiv_status
    path('HIVstatus/', views.hiv_status, name='hiv_status'),

    # HIV Risk Assessment Form
    re_path('hivscreeningtool/new/(?P<id>\d+)/',
            views.new_hivscreeningtool, name='new_hivscreeningtool'),

    # HIV Risk Management Form
    re_path('hivmanagementform/new/(?P<id>\d+)/',
            views.new_hivmanagementform, name='new_hivmanagementform'),

    # Dreams SerivceUptake Form
    re_path('dreamsform/new/(?P<id>\d+)/',
            views.new_dreamsform, name='new_dreamsform'),

    # New Graduation monitoring tool
    re_path('new_graduation_monitoring_form/new/(?P<id>\d+)/',
            views.new_graduation_monitoring_form, name='new_graduation_monitoring_form'),

    # Teen Snavyo
    re_path('new_sinovuyoteen_form/new/(?P<id>\d+)/',
            views.sinovuyoteenPreAndPostAssesmnetRevisedFinalDraft, name='new_sinovu_form'),
    
    path('new_sinovuyoteen_form_edit/<id>', views.edit_sinovuyoteenformrequest, name ='new_sinovuyoteen_form_edit'),
    
    
    # edit_teen_sinavuyo 
    re_path('new_sinovuyoteen_form_edit/(?P<id>\d+)/', views.edit_sinovuyoteenformrequest, name='new_sinovuyoteen_form_edit'),
    
    
    # delete_teen_sinavuyo
    re_path('new_sinovuyoteen_form_delete', views.delete_sinovuyoteenformrequest, name='new_sinovuyoteen_form_delete'),
    
#     new_sinovuyoteen_form_preview
    re_path('new_sinovuyoteen_form_preview', views.new_sinovuyoteenPreview, name='new_sinovuyoteen_form_preview'),
    
    
    
#     re_path('new_modelform_form/new/(?P<id>\d+)/',
#             views.create_view, name='new_sinovu_form_model_form'),
#     re_path('new_list_view_form/new/', views.list_view,
#             name='new_sinovu_form_liist_form'),
    #     re_path('new_list_view_form/new/', views.list_view, name='new_sinovu_form_liist_form'),
#     path('<id>/delete', views.update_view),
