...
from cpovc_forms.forms import (
    ...
    SinovuyoteenPreAndPostAssesmnetRevisedFinalDraftForm,
    SinovuyoteenPreAndPostAssesmnetRevisedFinalDraftFormEdit,
...
def sinovuyoteenPreAndPostAssesmnetRevisedFinalDraft(request, id):
    ...
    if request.method == 'POST':
        ...
        if data:
            event_type_id = 'WBGA'
            username = request.user.get_username()
            event_counter = OVCPreventiveEvents.objects.filter(event_type_id=event_type_id, person=id,
                                                            is_void=False).count()
            ovcpreventiveevent = OVCPreventiveEvents(
                event_type_id = event_type_id,
                event_counter = event_counter,
                event_score = 0,
                date_of_event = timezone.get_current_timezone(),
                date_of_previous_event = '2022-01-04',
                created_by = request.user.id,
                app_user = AppUser.objects.get(username=username),
                person = RegPerson.objects.get(pk=int(id)),
                house_hold = OVCHouseHold.objects.get(id=OVCHHMembers.objects.get(person=id).house_hold_id)
            )
            ovcpreventiveevent.save()
                   
            qry =OVCPrevSinovyoTeenEvaluation(
                    evaluation_id = str(ovcpreventiveevent.event),
                    person = str(id),
                    ref_caregiver = str(id),
    ...
    sinavuyo_teen_evaluation = OVCPrevSinovyoTeenEvaluation.objects.filter(event_id__in=event).order_by('assessment_date')
    return render(request=request, template_name='forms/new_sinovuyoteen.html', context={
        'child': child,
        'sinovuyo_teen':sinovuyoteen,
        'care_giver': care_giver,
        'house_hold': house_hold,
        'sinavuyo_teen_evaluation': sinavuyo_teen_evaluation
    })
    
    
def edit_sinovuyoteenformrequest(request, id):
    import pdb
    sinivuyo_teen = OVCPrevSinovyoTeenEvaluation.objects.get(person_id=id)
    data = request.POST
    child = RegPerson.objects.get(id=id)
    person = RegPerson.objects.get(id=id)
    # pdb.set_trace()
    ...
    if request.method == 'POST':
        ...
        OVCPrevSinovyoTeenEvaluation.objects.filter(evaluation_id=sinivuyo_teen.evaluation_id).update(                   
            person = child,
    ...
    sinivuyo_teen_data = { 
        ...
        'cb_figths_children' : sinivuyo_teen.dc_often_discipline ,
    ...
    return render(request, 'forms/new_sinovuyoteen.html', {'sinovuyo_teen': form, 'child': sinivuyo_teen, 'edit_flag': edit_flag, 'status': 200})
        



def delete_sinovuyoteenformrequest(request, id):
    import pdb
    child = RegPerson.objects.get(id=id)
    sinivuyo_teen = OVCPrevSinovyoTeenEvaluation.objects.get(person_id=id)
