...
from cpovc_forms.forms import (
    ...
    HIV_MANAGEMENT_ARV_THERAPY_FORM, HIV_MANAGEMENT_VISITATION_FORM, DREAMS_FORM, NewGraduationMonitoringForm, SinovuyoteenPreAndPostAssesmnetRevisedFinalDraftForm,
    oVCPrevSinovyoTeenEvaluationCreate,
    GeeksForm)
...
from .models import (
    ...
    OVCHIVManagement, OVCHIVRiskScreening, SinovuyuTeenPreAndPostAssesment, OVCPrevSinovyoTeenEvaluation,
    GeeksModel)
    ...
    try:
        if request.method == 'POST':
                ...
                if loggeduser_orgunitids:
                    ...
                    if ovccasegeos:
                        for ovccasegeo in ovccasegeos:
                            case_ids.append(str(ovccasegeo.case_id_id))
                    print(
                        'Get All Case for SuperUsers/Administrator/National Level WFC ... ')
            ...
            # Services & Moitoring(F1A Form)
                ...
                for person in personsets:
                    csi_data = OVCCareEvents.objects.filter(person=int(
                        person.id), event_type_id='FSAM', is_void=False)
                    if csi_data:
                        for csi in csi_data:
                            regperson = RegPerson.objects.get(pk=csi.person_id)
                            ### Add Person Attributes ###
                ...
                for person in personsets:
                    ...
                    try:
                        ovcreg = get_object_or_404(
                            OVCRegistration, person=int(person.id), is_void=False)
                        if ovcreg:
        ...
        if request.method == 'POST':
            ...
            if (action == 'EDU'):
                ...
                if admission_levels:
                    ...
                    print('existing_educationlevels - %s' %
                          existing_educationlevels)
...
def edit_placement(request, id):
    ...
    print('ovcplacement.residential_institution_name ... %s' %
          ovcplacement.residential_institution_name)
            ...
            if followup2_data:
                for followup2data in followup2_data:
                    ...
                    followup_outcome = 'Admitted : ' + \
                                       translate(
                                           followup2data.admitted_to_school)
...
def new_csi(request, id):
    try:
        if request.method == 'POST':
            ...
            my_kvals.append(
                {"entity": "HES1", "value": household_strengthening})
...
def edit_csi(request, id):
    try:
        if request.method == 'POST':
            ...
            for kvals in my_kvals:
                ...
                value = kvals["value"]
                attribute = "FCSI"
                ovccareeavs = OVCCareEAV.objects.filter(
                    event=id, entity=key, is_void=False)
            ...
            if olmis_priority_service_provided_list:
                ...
                existingprioritys = OVCCarePriority.objects.filter(
                    event=id, is_void=False)
                ...
                for existing_priority in existing_prioritys:
                    ...
                    if (eservice not in nservices):
                        # delete service
                        print('eservice (%s), service_grouping_id (%s)' %
                              (eservice, service_grouping_id))
            ...
            if olmis_service_provided_list:
                ...
                existingservices = OVCCareServices.objects.filter(
                    event_id=id, is_void=False)
    ...
    csi_eav_data = OVCCareEAV.objects.filter(
        event=id, is_void=False).values('entity', 'value').order_by('entity')
    eavdata = []
    ...
    for svc_grouping_id in svc_grouping_ids:
        csi_services = csi_services_data.filter(
            service_grouping_id=svc_grouping_id)
        for csi_service in csi_services:
            setuplist = SetupList.objects.get(
                item_id=csi_service.service_provided, item_category='Service')
            field_name = setuplist.field_name
            setuplist2 = SetupList.objects.get(
                item_sub_category=field_name, item_category='Domain')
...
def view_csi(request, id):
    ...
    try:
        ...
        # get priority data
        csi_priority_data = OVCCarePriority.objects.filter(
            event=id, is_void=False)
        jsonPrData = []
        resultsetspr = []
        pr_grouping_ids = []
        ...
        for pr_grouping_id in pr_grouping_ids:
            services = []
            pr_needs = csi_priority_data.filter(
                service_grouping_id=pr_grouping_id)
            for pr_need in pr_needs:
                services.append(str(pr_need.service))
                domain = pr_need.domain
        ...
        # get services data
        csi_services_data = OVCCareServices.objects.filter(
            event=id, is_void=False)
        jsonSvcData = []
        resultsetssvc = []
        svc_grouping_ids = []
...
def update_event_date(pk, date_of_assessment):
    # Save F1AEvent
    OVCCareEvents.objects.filter(pk=pk).update(
        date_of_event=date_of_assessment)
...
def update_form1a(request):
    ...
    try:
        if request.method == 'POST':
            ...
            event_obj = OVCCareEvents.objects.get(
            ...
            if args == 1:
                ...
                ovc_care_assessment = OVCCareAssessment.objects.filter(event=event_obj)[
                                                                       :1]
                # F1A Assessment
                olmis_assessment_provided_list = request.POST.get(
                    'olmis_assessment_provided_list')
            ...
            if args == 3:
                ...
                event_obj = OVCCareEvents.objects.get(
                    pk=request.POST.get('event_pk'))
                ovc_care_priority = OVCCarePriority.objects.filter(event=event_obj)[
                                                                   :1]
                # Priority Needs
                olmis_priority_service_provided_list = request.POST.get(
                    'olmis_priority_service_provided_list')
            ...
            if args == 4:
                ...
                if date_of_service:
                    date_of_service = convert_date(date_of_service)
                ovc_care_services = OVCCareServices.objects.filter(event=event_obj)[
                                                                   :1]
                # Support/Services
                olmis_service_provided_list = request.POST.get(
                    'olmis_service_provided_list')
...
def edit_form1a(request, id, btn_event_type, btn_event_pk):
    ...
    if delta < 90:
        if btn_event_type == 'ASSESSMENT':
            ovc_care_assessments = OVCCareAssessment.objects.filter(
                event=event_obj)
            ...
            for ovc_care_assessment in ovc_care_assessments:
                ...
                assessment_entry.append(
                    translate(ovc_care_assessment.service_status))
            ...
            for critical_event in critical_events:
                if loop_count == 0:
                    critical_events_lst = critical_events_lst + \
                        str(critical_event.value)
            ...
            services_list = []
            # get Services
            ovccareservices = OVCCareServices.objects.filter(
                event=event_obj, is_void=False)
            ...
            olmis_domain_list = get_list('olmis_domain_id', 'Please Select')
            # get Prioritys
            ovcprioritys = OVCCarePriority.objects.filter(
                event=event_obj, is_void=False)
...
def manage_form1a_events(request):
    ...
    try:
        ...
        for ovccareevent in ovccareevents:
            ...
            # get Assessment
            ovccareassessments = OVCCareAssessment.objects.filter(
                event=ovccareevent.pk)
            ...
            # get CriticalEvents
            ovccriticalevents = OVCCareEAV.objects.filter(
                event=ovccareevent.pk)
...
def manage_form1b_events(request):
    ...
    try:
        person = request.POST.get('person')
        ovccareevents = OVCCareEvents.objects.filter(
            person=person, event_type_id='FM1B')
        for ovccareevent in ovccareevents:
            event_type = None
            event_details = None
            services = []
...
def new_hhva(request, id):
    try:
        if request.method == 'POST':
            ...
            # ************************************************************
            my_kvals.append({"entity": "HA1M", "attribute": "HA1M",
                            "value": hhva_ha1_male, "value_for": ''})
            my_kvals.append({"entity": "HA1F", "attribute": "HA1F",
                            "value": hhva_ha1_female, "value_for": ''})
            my_kvals.append({"entity": "HA2M", "attribute": "HA2M",
                            "value": hhva_ha2_male, "value_for": ''})
            my_kvals.append({"entity": "HA2F", "attribute": "HA2F",
                            "value": hhva_ha2_female, "value_for": ''})
            my_kvals.append({"entity": "HA3M", "attribute": "HA3M",
                            "value": hhva_ha3_male, "value_for": ''})
            my_kvals.append({"entity": "HA3F", "attribute": "HA3F",
                            "value": hhva_ha3_female, "value_for": ''})
            my_kvals.append({"entity": "HA4M", "attribute": "HA4M",
                            "value": hhva_ha4_male, "value_for": ''})
            my_kvals.append({"entity": "HA4F", "attribute": "HA4F",
                            "value": hhva_ha4_female, "value_for": ''})
            ...
            # ************************************************************
            my_kvals.append({"entity": "HA5", "attribute": "HA5",
                            "value": hhva_ha5, "value_for": ''})
            for i, ha6 in enumerate(hhva_ha6):
                ha6 = ha6.split(',')
                for value in ha6:
                    my_kvals.append(
                        {"entity": "HA6", "attribute": "HA6", "value": value, "value_for": ''})
            my_kvals.append({"entity": "HA7", "attribute": "HA7",
                            "value": hhva_ha7, "value_for": ''})
            my_kvals.append({"entity": "HA8", "attribute": "HA8",
                            "value": hhva_ha8, "value_for": ''})
            ...
            # ************************************************************
            my_kvals.append({"entity": "HA9", "attribute": "HA9",
                            "value": hhva_ha9, "value_for": ''})
            for data in hhva_wash_data:
                type_ = data["type"]
                condition = data["condition"]
                number = data["number"]
                my_kvals.append(
                    {"entity": 'HA10', "attribute": type_, "value": number, "value_for": 'NUMBER'})
                my_kvals.append({"entity": 'HA10', "attribute": type_,
                                "value": condition, "value_for": 'CONDITION'})

            # Food Security & Nutrition
            hhva_ha11 = request.POST.get('hhva_ha11')
            hhva_ha12 = request.POST.get('hhva_ha12')
            # ************************************************************
            my_kvals.append({"entity": "HA11", "attribute": "HA11",
                            "value": hhva_ha11, "value_for": ''})
            my_kvals.append({"entity": "HA12", "attribute": "HA12",
                            "value": hhva_ha12, "value_for": ''})
            ...
            for data in hhva_asset_data:
                ...
                size = data["size"]
                my_kvals.append(
                    {"entity": 'HA15', "attribute": asset, "value": number, "value_for": 'NUMBER'})
                my_kvals.append(
                    {"entity": 'HA15', "attribute": asset, "value": size, "value_for": 'SIZE'})
            my_kvals.append({"entity": "HA13", "attribute": "HA11",
                            "value": hhva_ha13, "value_for": ''})
            my_kvals.append({"entity": "HA14", "attribute": "HA14",
                            "value": hhva_ha14, "value_for": ''})
            my_kvals.append({"entity": "HA16", "attribute": "HA16",
                            "value": hhva_ha16, "value_for": ''})
            my_kvals.append({"entity": "HA17", "attribute": "HA17",
                            "value": hhva_ha17, "value_for": ''})
            my_kvals.append({"entity": "HA18", "attribute": "HA18",
                            "value": hhva_ha18, "value_for": ''})
            my_kvals.append({"entity": "HA19", "attribute": "HA19",
                            "value": hhva_ha19, "value_for": ''})
            my_kvals.append({"entity": "HA20", "attribute": "HA20",
                            "value": hhva_ha20, "value_for": ''})
            for i, ha21 in enumerate(hhva_ha21):
                ha21 = ha21.split(',')
                for value in ha21:
                    my_kvals.append(
                        {"entity": "HA21", "attribute": "HA21", "value": value, "value_for": ''})
            ...
            # ************************************************************
            my_kvals.append({"entity": "HA22", "attribute": "HA22",
                            "value": hhva_ha22, "value_for": ''})
            my_kvals.append({"entity": "HA23", "attribute": "HA23",
                            "value": hhva_ha23, "value_for": ''})
            my_kvals.append({"entity": "HA24", "attribute": "HA24",
                            "value": hhva_ha24, "value_for": ''})
            my_kvals.append({"entity": "HA25", "attribute": "HA25",
                            "value": hhva_ha25, "value_for": ''})
            my_kvals.append({"entity": "HA26M", "attribute": "HA26M",
                            "value": hhva_ha26_male, "value_for": ''})
            my_kvals.append({"entity": "HA26F", "attribute": "HA26F",
                            "value": hhva_ha26_female, "value_for": ''})
            my_kvals.append({"entity": "HA27M", "attribute": "HA27M",
                            "value": hhva_ha27_male, "value_for": ''})
            my_kvals.append({"entity": "HA27F", "attribute": "HA27F",
                            "value": hhva_ha27_female, "value_for": ''})
            ...
            for i, ha28 in enumerate(hhva_ha28):
                ...
                for value in ha28:
                    my_kvals.append(
                        {"entity": "HA28", "attribute": "HA28", "value": value, "value_for": ''})
            ...
            for i, ha29 in enumerate(hhva_ha29):
                ...
                for value in ha29:
                    my_kvals.append(
                        {"entity": "HA29", "attribute": "HA29", "value": value, "value_for": ''})
            for i, ha30 in enumerate(hhva_ha30):
                ha30 = ha30.split(',')
                for value in ha30:
                    my_kvals.append(
                        {"entity": "HA30", "attribute": "HA30", "value": value, "value_for": ''})
            ...
            for i, ha31 in enumerate(hhva_ha31):
                ...
                for value in ha31:
                    my_kvals.append(
                        {"entity": "HA31", "attribute": "HA31", "value": value, "value_for": ''})
    ...
    try:
        ovcreg = get_object_or_404(
            OVCRegistration, person_id=id, is_void=False)
        caretaker_id = ovcreg.caretaker_id if ovcreg else None
        ovchh = get_object_or_404(
            OVCHouseHold, head_person=caretaker_id, is_void=False)
        household_id = ovchh.id if ovchh else None
        ...
        if request.method == 'POST':
            ...
            if domain_id:
                ...
                if index == 2:
                    ...
                    print('assessmentcategory.item_sub_category -- %s' %
                          assessmentcategory.item_sub_category)
...
def manage_form_type(request):
    ...
    try:
        ...
        if (reg_ovc) or (user_id == 1):
            d = dict(all_forms)
            jsonFormTypeData = [{"value": i, "label": j}
...
def new_cpara(request, id):
    ...
    cpara_events = OVCCareEvents.objects.filter(
        event_type_id='cpr', person_id=child.id)
    if cpara_events:
        for one_cpara_event in cpara_events:
            event_detail = ""
            total_benchmark_score = 0
            # cpara_data = OVCCareCpara.objects.filter(event=one_cpara_event)
            cpara_data = OVCCareBenchmarkScore.objects.filter(
                event_id=one_cpara_event.event)
            bm_array = []
            if cpara_data:
                for one_cpara_bench in cpara_data:
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_1 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_2 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_3 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_4 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_5 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_6 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_7 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_8 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_9 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_10 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_11 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_12 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_13 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_14 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_15 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_16 == 1 else "No")
                    bm_array.append(
                        "Yes" if one_cpara_bench.bench_mark_17 == 1 else "No")
                    ...
                    str_1 = benchmark_1 + ", " + benchmark_2 + ", " + \
                        benchmark_3 + ", " + benchmark_4 + ", " + benchmark_5 + ", "
                    str_2 = benchmark_6 + ", " + benchmark_7 + \
                        ", " + benchmark_8 + ", " + benchmark_9 + ", "
                    str_3 = benchmark_10 + ", " + benchmark_11 + \
                        ", " + benchmark_12 + ", " + benchmark_13 + ", "
                    str_4 = benchmark_14 + ", " + benchmark_15 + \
                        ", " + benchmark_16 + ", " + benchmark_17
            ...
            for all_data in caseplandata:
                ...
                for service in my_service:
                    OVCCareCasePlan(
                        ...
                        date_of_event=convert_date(
                            my_date_of_caseplan, fmt='%Y-%m-%d'),
                        form=OVCCareForms.objects.get(name='OVCCareCasePlan'),
                        completion_date=convert_date(
                            my_date_completed, fmt='%Y-%m-%d'),
                        actual_completion_date=convert_date(
                            my_actual_completion_date, fmt='%Y-%m-%d'),
                        results=my_results,
                        reasons=my_reason,
                        case_plan_status='D',
                        event=ovccareevent
    ...
    if request.method == 'POST':
        ...
        if delta < 90:
            try:
                ...
                care_giver = RegPerson.objects.get(
                    id=OVCRegistration.objects.get(person=child).caretaker_id)
                caregiver_id = OVCRegistration.objects.get(
                    person=child).caretaker_id
                ...
                if my_request:
                    ...
                    xyz = RegPerson.objects.filter(
                        id=caregiver_id).values('id')
...
def new_case_plan_monitoring(request, id):
    if request.method == 'POST':
        ...
        event = OVCCareEvents.objects.filter(
            person_id=ovc_id).values_list('event')
        cpara_mon_data = OVCMonitoring.objects.filter(
            event_id__in=event).order_by('event_date')
...
def persist_wellbeing_data(kvals, value, person, house_hold, new_pk, date_of_wellbeing_event, request, id):
    ...
    other_list = ['WB_SAF_1_2', 'WB_SAF_31_2', 'WB_SAF_34_2', 'WB_SAF_37_2', 'WB_SAF_38_2', 'WB_SAF_39_2', 'WB_SAF_40_2', 'WB_SCH_41_2', 'WB_STA_1_2', 'WB_STA_2_2', 'WB_STA_3_2',
    ...
    demographics_items = ['WB_GEN_12', 'WB_GEN_13',
        'WB_GEN_15', 'WB_GEN_06', 'WB_GEN_08']
    # try:
    ...
    if (key == 'safeanswer'):
        ...
        for person_id, individual_person_answers in answer_obj.items():
            ...
            for element_id, answer in individual_person_answers.items():
                kvals = {"entity": "wellbeing", "value": answer,
                    "question_code": element_id}
...
def new_wellbeing(request, id):
    ...
    if request.method == 'POST':
        ...
        event_type_id = 'WBG'
        date_of_wellbeing_event = convert_date(
            request.POST.get('WB_GEN_01'), fmt='%Y-%m-%d')
        ...
        for key in request.POST:
            if (str(key) != "safeanswer" and str(key) != "schooledanswer" and "WB_SCH_39" not in str(
                ...
                persist_per_child_wellbeing_question(
                    request, key, house_hold, new_events_pk)
        url = reverse('ovc_view', kwargs={'id': id})
    ...
    form = Wellbeing(
        initial={'household_id': household_id, 'caretaker_id': caretaker_id, })
    care_giver = RegPerson.objects.get(
        id=OVCRegistration.objects.get(person=child).caretaker_id)
...
def hiv_status(request):
    try:
        if request.method == 'POST':
            ...
            house_hold = OVCHouseHold.objects.get(
                id=OVCHHMembers.objects.get(person=child_id).house_hold_id)
            event_type_id = 'HIVS'
            event_counter = OVCCareEvents.objects.filter(
    ...
    if request.method == 'POST':
        try:
            ...
            if True:
                ...
                if facility:
                    facility_res = OVCFacility.objects.get(
                ...
                ovcscreeningtool = OVCHIVRiskScreening.objects.create(
                    ...
                    test_done_when=data_to_save.get(
                        'HIV_RS_03'),  # date of assesment
        ...
        hiv_screen = OVCHIVRiskScreening.objects.filter(
            event_id__in=event).order_by('date_of_event')
...
def new_hivmanagementform(request, id):
    if request.method == 'POST':
        ...
        qry = OVCHIVManagement(
            ...
            treatment_supporter_relationship=request.POST.get(
                'HIV_MGMT_2_H_1'),
            ...
            detectable_viralload_interventions=request.POST.get(
                'HIV_MGMT_2_J'),
        ...
        try:
            ...
            check_fields = ['sex_id']
            vals = get_dict(field_name=check_fields)
            form_arvtherapy = HIV_MANAGEMENT_ARV_THERAPY_FORM(
                initial={'person': id})
            child_hiv_status = OVCRegistration.objects.get(
                person=id).hiv_status
...
def new_graduation_monitoring_form(request, id):
    if request.method == 'POST':
        recommended_action = NewGraduationMonitoringForm(
            request.POST).RecommendActionForm(request.POST)
        new_graduation_form_month_1 = NewGraduationMonitoringForm(
            request.POST, prefix="month_1")
        new_graduation_form_month_2 = NewGraduationMonitoringForm(
            request.POST, prefix="month_2")
        new_graduation_form_month_3 = NewGraduationMonitoringForm(
            request.POST, prefix="month_3")
        child = RegPerson.objects.get(id=id)
        if new_graduation_form_month_1.is_valid() and new_graduation_form_month_2.is_valid() and new_graduation_form_month_3.is_valid():
            ...
            messages.success(request, 'Your property was successfully added!')
        else:
            messages.error(request, 'Error saving form!')

        render(request=request, template_name='forms/new_graduation.html',
...
def sinovuyoteenPreAndPostAssesmnetRevisedFinalDraft(request, id):
    ...
    if request.method == 'POST':
        ...
        data_assessment = data['assessment_date']

        sinovuyoteenform = oVCPrevSinovyoTeenEvaluationCreate(request.POST)
        if sinovuyoteenform.is_valid():
            sinovuyoteenform.save()
            messages.success(request, 'Form Saved Successfully!')
        else:
            messages.error(request, 'Error saving form!')

        render(request=request, template_name='forms/new_sinovuyoteen.html',
               context={'sinovuyo_teen': sinovuyoteenform})

    sinovuyoteen = SinovuyoteenPreAndPostAssesmnetRevisedFinalDraftForm()
...
# relative import of forms
   context["form"] = form
    ...
    return render(request, "forms/update_view.html", context)
...
def delete_view(request, id):
    ...
    return render(request, "forms/delete_view.html", context)
