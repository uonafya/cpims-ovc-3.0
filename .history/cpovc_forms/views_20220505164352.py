...
def sinovuyoteenPreAndPostAssesmnetRevisedFinalDraft(request, id):
    ...
    person = RegPerson.objects.get(id=id)
    # check_fields = ['sex_id','school_level_id', 'yesno_id','school_type_id']
    check_fields = ['feeling_sad_id']
    vals = get_dict(field_name=check_fields)
    pdb.set_trace()
...
def edit_sinovuyoteenformrequest(request, id):
    ...
    edit_flag = True
    care_giver = RegPerson.objects.get(id=OVCRegistration.objects.get(person=child).caretaker_id)
    sinovuyoteen = SinovuyoteenPreAndPostAssesmnetRevisedFinalDraftFormEdit(data=sinivuyo_teen_data)
    # pdb.set_trace()
    # call beb after    
    return render(request, 'forms/new_sinovuyoteen.html', {
        'sinovuyo_teen':sinovuyoteen,
        'care_giver': care_giver,
        'child': child, 
        'edit_flag': edit_flag, 
