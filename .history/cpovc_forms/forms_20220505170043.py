...
from .models import (OVCPrevSinovyoTeenEvaluation)
...
class SinovuyoteenPreAndPostAssesmnetRevisedFinalDraftForm(forms.ModelForm):
    CHOICES_READ = (("", 'Select'),
    ...
    Mother_Mortality_choices = (("", 'Select'),
        (0, 'Yes'), (1, 'No, she passed away'), (2, 'No, she lives elsewhere'))

    Father_Mortality_choices = (("", 'Select'),
        (0, 'Yes'), (1, 'No, he passed away'), (2, 'No, he lives elsewhere'))

    SECTION_TWO = (("", 'Select'),(0, 'Never'), (1, 'Almost never'),
                   (2, 'Sometimes'), (3, 'Often'), (4, 'Always'))

    SECTION_THREE_MY_BEHAVIOUR = (("", 'Select'),(0, 'Not true'), (1, 'Somewhat true'), (2, 'Very true'))

    SECTION_DISCIPLINE_FROM_MY_CAREGIVER = (("", 'Select'),(0, 'Never'),
                                            (1, '1 time'),
                                            (2, '2 times'),
                                            (3, '3 times'),
                                            (4, '4 times'),
    ...
    SECTION_FEELING_SAD = (("", 'Select'),(0, 'Rarely or none of the time less than 1 day'),
    ...
    assessment_date = forms.DateField(widget=AdminDateWidget())

    bd_age = forms.IntegerField(max_value=18, min_value=1)
    
    bd_sex = forms.ChoiceField(
        choices=sex_id_list_teen,
        widget=forms.RadioSelect()
    )
    ...
    bd_bd_boarding_status = forms.ChoiceField(
        choices=school_admission_type_list,
        widget=forms.Select(
            attrs={
    ...
    sp_physical_punish_choices =((0,'Strongly disagree'),
                                 (1,'Disagree'),
                                 (2,'Neutral (neither agree or disagree'),
                                 (3,'Agree'),
                                 (4,'Disagree'))
    
    sp_physical_punish = forms.ChoiceField(
        choices=sp_physical_punish_choices,
        widget=forms.Select(
            attrs={
                'placeholder': 'Live with Biological father....',
                'class': 'form-control',
    ...
    fs_too_tired = forms.ChoiceField(
        choices=SECTION_FEELING_SAD,
        widget=forms.Select(
            attrs={
                'placeholder': 'Live with Biological father....',
    ...
    fmp_pre_grouping_id = forms.ChoiceField(
        choices=SECTION_FEELING_SAD,
        widget=forms.Select(
            attrs={
                'placeholder': 'Live with Biological father....',
    ...
    class Meta:
        ...
        model = OVCPrevSinovyoTeenEvaluation
...
#>>>>>>>>>>>>>>>>>>>teen sinovuyo>>>>>>>>>>>>>>>>
yesno_list_teen = get_list('yesno_id')
bd_education_level  = get_list('school_level_id','Please Select')
sex_id_list_teen = get_list('sex_id')

SECTION_DISCIPLINE_FROM_MY_CAREGIVER = get_list('dsp_times_id')
school_type_id = get_list('school_type_id')
SECTION_FEELING_SAD = get_list('feeling_sad_id')
SECTION_THREE_MY_BEHAVIOUR = get_list('my_behaviour_id')
SECTION_TWO = get_list('relationship_caregiver_id')
Father_Mortality_choices = get_list('father_mortality_id')
Mother_Mortality_choices = get_list('dsp_times_id')
CHOICES_READ = get_list('literacy_lvl_id')
# dsp_times_id = get_list('dsp_times_id')
        
class SinovuyoteenPreAndPostAssesmnetRevisedFinalDraftFormEdit(forms.ModelForm):
    CHOICES_READ = ((0, 'Cannot read at all'),(0, 'Cannot read at all'),
                    (1, 'Can read but with lots of difficulty'),
                    (2, 'Can read with a little difficulty'),
                    (3, 'Can read easily'))
    ...
    ASSESSMENT_TYPE=forms.ChoiceField(
        choices=((1, 'Pre-Programme'), (0, 'Post-Programme')),
        widget=forms.RadioSelect( 
                attrs={
                    'readonly': 'readonly',
                }                     
...
class oVCPrevSinovyoTeenEvaluationCreate(forms.ModelForm):
    class Meta: 
        model = OVCPrevSinovyoTeenEvaluation
        fields = '__all__'
