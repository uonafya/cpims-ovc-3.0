...
@login_required
def reports_prev(request, id):
    """Method to do pivot reports."""
    try:
        rpt_id = int(id)
        name = PR_RPTS[rpt_id] if rpt_id in PR_RPTS else PR_RPTS[1]
        form = CaseLoad(request.user)
        return render(request, 'reports/pivot_ovc_other.html',
                      {'form': form, 'name': name,
                       'report_id': rpt_id, 'report_ovc': 7})
    except Exception as e:
        raise e
    else:
        pass
...
@login_required
def reports_pmtct(request, id):
    """Method to do pivot reports."""
    try:
        rpt_id = int(id)
        name = PM_RPTS[rpt_id] if rpt_id in PM_RPTS else PM_RPTS[1]
        form = CaseLoad(request.user)
        return render(request, 'reports/pivot_ovc_other.html',
                      {'form': form, 'name': name,
                       'report_id': rpt_id, 'report_ovc': 8})
    except Exception as e:
        raise e
    else:
        pass
