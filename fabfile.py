...
def install_cpims():
    ...
    local(
        'tar --exclude=cpims/configs --exclude=cpims/.git --exclude=.gitignore --exclude=*pyc --exclude=fabfile.py* -C %s -czvf %s/cpims.tar.gz cpims' % (
        src_dir, deploy_dir,))
...
def setup_pg():
    ...
    with settings(sudo_user='postgres') and cd('/var/lib/pgsql'):
        sudo("psql -c \"create user %s with encrypted password '%s'\"" % (cpims_dbuser, cpims_password),
             user='postgres')
