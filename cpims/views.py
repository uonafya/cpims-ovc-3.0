...
def get_ovc_active_hiv_status(request, org_level, area_id):
    ...
    return JsonResponse(main_dash_data, content_type='application/json',safe=False)
...
def fetch_cbo_list(request):
    return JsonResponse(get_cbo_list(), content_type='application/json', safe=False)


def get_locality_data(request):
    print("locality data")
    locality_data = fetch_locality_data()
    return JsonResponse(locality_data, content_type='application/json',safe=False)
