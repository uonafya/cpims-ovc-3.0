...
from cpovc_forms.forms import (
    ...
    gradMonitoringToolform, CaseTransferForm,
    BenchmarkMonitoringForm, CaseClosureForm)
...
from .forms import CaseClosureForm
    ...
    try:
        ...
        return render(request,
                      ...
                      {'form': form, 'data': init_data,
                       ...
                       'ev_data': ev_data, 'caregiver': care_giver,
                       'form1b_allowed': f1b_allow, 'f1bs': f1bs})
    except Exception as e:
        url = reverse('ovc_view', kwargs={'id': id})
        msg = 'Missing Caregiver Error - %s' % str(e)
        messages.add_message(request, messages.ERROR, msg)
        return HttpResponseRedirect(url)
    else:
        pass
...
def manage_form1a_events(request):
    ...
    try:
        ...
        ovccareevents = OVCCareEvents.objects.filter(
            person=person, event_type_id='FSAM', is_void=False).order_by('-date_of_event')
...
def new_case_closure(request, id):
    try:
        ovc_id = int(id)
        user_id = request.user.id
        ovc = OVCRegistration.objects.get(person_id=ovc_id)
        caregiver_id = ovc.caretaker_id
        cbo_id = ovc.child_cbo_id
        exit_reason = ovc.exit_reason
        print(exit_reason)
        reason = 'ERCT' if exit_reason in ['ERFO', 'ERTL', 'ERDG'] else 'ERAT'
        if exit_reason == 'ERCP':
            reason = 'ERCP'
        initial = {'CASE_CL001A': reason}
        exit_org = get_exit_org(ovc_id, True)
        ou_name, ou_id = '', ''
        if exit_org:
            ou_id = exit_org.org_unit_id
            ou_name = exit_org.org_unit_name
        initial['CASE_CL001B'] = ou_name
        initial['exit_org_id'] = ou_id
        initial['CASE_CL001C'] = exit_reason
        form = CaseClosureForm(initial=initial)

        if request.method == 'POST':
            data = request.POST
            print(data)
            closure_reason = request.POST.get("CASE_CL001A")
            # Default to NULL when users switches btn
            # CPA/Transfer/Attrition
            files_completed = None
            phone_number = None
            informed_graduation = None
            head_hh_linked = None
            ...
            transfer_completed = None
            followup_time = None
            informed_providers = None
            copy_sent = None
            reason_other = None
            attrition_documented = None
            manager_report = None
            reason_documented = None
            attrition_reason = None
            attrition_reason_other = None
            # Other
            file_stored = request.POST.get("CASE_CL013")
            closure_date = request.POST.get("CASE_CL014")
            receiving_org_name = request.POST.get("CASE_CL001B")
            receiving_org_id = request.POST.get("exit_org_id")
            # Date
            event_date = convert_date(closure_date)
            # Overwrite
            if closure_reason == 'ERCP':
                # CPA
                files_completed = request.POST.get("CASE_CL002")
                phone_number = request.POST.get("CASE_CL003")
                informed_graduation = request.POST.get("CASE_CL004")
                head_hh_linked = request.POST.get("CASE_CL005")
            elif closure_reason == 'ERCT':
                # Transfers
                transfer_completed = request.POST.get("CASE_CL006")
                followup_time = request.POST.get("CASE_CL007")
                informed_providers = request.POST.get("CASE_CL008")
                copy_sent = request.POST.get("CASE_CL009")
            elif closure_reason == 'ERAT':
                attrition_reason = request.POST.get("CASE_CL001C")
                attrition_reason_other = request.POST.get("CASE_CL001D")
                attrition_documented = request.POST.get("CASE_CL010")
                manager_report = request.POST.get("CASE_CL011")
                reason_documented = request.POST.get("CASE_CL012")
            """ Save Form details """
            defaults = {'reason': closure_reason, 'caregiver_id': caregiver_id,
                        ...
                        'attrition_reason_other': attrition_reason_other,
                        'transfer_form_completed': transfer_completed,
                        'follow_up_frequency': followup_time,
                        'sp_informed_transfer': informed_providers,
                        'family_folder_sent': copy_sent,
                        'attrition_documented': attrition_documented,
                        'manager_report': manager_report,
                        'exit_reason_stored': reason_documented,
                        'rec_organization_name': receiving_org_name,
                        'rec_organization_id': receiving_org_id,
                        'files_stored': file_stored, 'date_of_closure': event_date,
                        'organization_id': cbo_id, 'created_by_id': user_id}
            closure, created = OVCCareCaseExit.objects.update_or_create(
                    person_id=ovc_id, defaults=defaults)

            msg = "OVC case closure / exit filled successfully"
            messages.info(request, msg)
            url = reverse('ovc_view', kwargs={'id': ovc_id})
            return HttpResponseRedirect(url)
        else:
            vals = {}
            case_closure = OVCCareCaseExit.objects.filter(person_id=ovc_id)
            context = {'ovc': ovc, 'form': form, 'case_closure': case_closure, 'vals': vals}
...
def edit_case_closure(request, id, cid):
    """Some default page for Server Errors."""

    try:

        posted_data = OVCCareCaseExit.objects.get(pk=cid, is_void=False)
        person_id = posted_data.person_id
        if request.method == 'POST':
        ...
        gotten_data = {
            ...
            'CASE_CL012': posted_data.sp_informed_transfer,
...
def edit_grad_monitor(request, id):
    if request.method=='POST':
        ...
        household1 =OVCBenchmarkMonitoring.objects.get(obm_id=id)
        re_person = OVCHHMembers.objects.filter(
            house_hold=household1.household, hh_head=False, member_type = 'TOVC').first().person.id
