...
from cpovc_dashboard.models import IPInfo
...
def get_chart_data(request, rid, county_id, const_id,
    ...
    try:
        ...
        params['r_period'] = dates['r_period']
        params['r_year'] = dates['r_year']
...
def get_raw_data(rid, params):
    ...
    try:
        ...
        # Flat table filters
        ftdates, oftdates = '', ''
        rp = params['r_period'] if 'r_period' in params else None
        ry = params['r_year'] if 'r_year' in params else None
        if rp and ry:
            ftdates = "AND r_period = %s AND r_year = %s" % (rp, ry)
            oftdates = "WHERE r_period = %s AND r_year = %s" % (rp, ry)
        qparams['dates'] = dquery
        qparams['fdate'] = fdates
        qparams['odate'] = odate
        qparams['odates'] = odates
        # FT filters
        qparams['ftdates'] = ftdates
        qparams['oftdates'] = oftdates
