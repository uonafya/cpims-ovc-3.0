...
from .models import (
    OVCMobileEvent, OVCMobileEventAttribute, CasePlanTemplateEvent,
    CasePlanTemplateService, OVCEvent, OVCServices, OVCMobileEventRejected,
    OVCEventRejected, OVCMobileEventAttributeRejected, OVCServicesRejected,
...
def mobile_stats(request):
    """Method to do Mobile Stats."""
    try:
        form_id = request.GET.get('form_id', 'All Forms')
        stats = get_events(request)
        context = {'stats': stats}
        ...
        return render(
            request, 'mobile/stats.html', context)
...
def mobile_devices(request):
    ...
    try:
        ...
        return render(
            request, 'mobile/device.html', context)


def mobile_geos(request):
    """Method to get Mobile App Geos."""
    try:
        data = []
        tst = {"name": "Nairobi", "lat": -1.2790791780104867, "lon": 36.81583600793639, "CPIMS_ID": "000"}
        for i in range(1, 100):
            lat = round(random.uniform(-4,4), 20)
            lon = round(random.uniform(34,41), 20)
            pnt = {"name": "Area %s" % i, "lat": lat, "lon": lon, "CPIMS_ID": i}
            data.append(tst)

    except Exception as e:
        return JsonResponse({'message': 'Error %s' % str(e)}, safe=False)
    else:
