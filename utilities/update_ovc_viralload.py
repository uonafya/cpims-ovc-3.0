        ...
        try:
            for chunk in chunks:
                ...
                for row in chunk:
                    ...
                    viral_load_records = self.get_viral_load(
                    ...
                    if viral_load_records:
                        for record in viral_load_records:
                            ...
                            if OVCViralload.objects.filter(
                                    person_id=person_id,
                                    viral_date=date_tested,
                                    viral_load__isnull=True).exists():
                                OVCViralload.objects.filter(
                                    person_id=person_id,
                                    viral_date=date_tested,
                                    viral_load__isnull=True).update(
                                    viral_load=result_to_int)
                            # check if record exists before inserting
                            elif OVCViralload.objects.filter(
                                    person_id=person_id,
                                    viral_date=date_tested).exists() == False:
                                vl = OVCViralload(
                                    person_id=person_id,
                                    viral_load=result_to_int,
                                    viral_date=date_tested)
