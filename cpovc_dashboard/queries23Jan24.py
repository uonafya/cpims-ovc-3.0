...
vw_cpims_dash_caseload.exit_reason not in (
'Transferred to PEPFAR partner', 'Transferred to Non-PEPFAR partner', 'Transfered to GOK - DCS')
...
AND date_of_event < '2023-10-01' {cbos}
UNION
Select count(distinct(cpims_ovc_id)) as dcount,
...
where benchmark_1 = 0 and date_of_event > '2022-03-31' {cbos}
...
where benchmark_2 = 0 and date_of_event > '2022-03-31' {cbos}
...
where benchmark_5 = 0 and date_of_event > '2022-03-31' {cbos}
...
where benchmark_6 = 0 and date_of_event > '2022-03-31' {cbos}
...
where benchmark_9 = 0 and date_of_event > '2022-03-31' {cbos}
...
Select count(distinct(person_id)) AS dcount,
       ...
       CASE
           ...
           when ovchivstatus='HIV Referred For Testing' THEN 'HIV Status Unknown'
           ELSE 'Others' END AS hivstat
from vw_cpims_dash_hivstat where person_id in
                                      (select distinct(x.person_id) from
                                          (
                                              Select distinct person_id
                                              from vw_Active_Beneficiary_APR22 {ocbos}
...
and agency is not null and cbo is not NULL and county is not NULL
