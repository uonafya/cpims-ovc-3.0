...
--where agerange NOT IN ('g.[21+yrs]') {ocbos}
UNION
select count(distinct(cpims_ovc_id)) as dcount,
...
'Male' as sex_id, 'On ART' as hivstat
from vw_cpims_registration_fy22 where exit_status='ACTIVE'
and ovchivstatus='POSITIVE' AND agerange != 'g.[21+yrs]'  {cbos}UNION
Select count(distinct(v.cpims_ovc_id)) AS dcount,
'Male' as sex_id, 'Suppression' as hivstat
...
QUERIES['3H'] = '''
Select count(distinct(cpims_ovc_id)) as dcount, gender as sex_id, agerange
from vw_cpims_dash_viral_load where exit_status='ACTIVE' AND
--(current_date - date_of_event) < 401
vl_period_validity='Valid'
and viral_load > 199 AND agerange != 'g.[21+yrs]'
AND EXISTS (SELECT person_id FROM vw_cpims_dash_ovc_serv WHERE
vw_cpims_dash_viral_load.cpims_ovc_id = vw_cpims_dash_ovc_serv.person_id)
...
from vw_cpims_case_plan where ('2022-09-30' - date_of_event) <= 455
AND date_of_event < '2022-09-30' {cbos}
UNION
...
Select count(distinct(household)) as dcount,
'CPARA' as services, 'SMAL' as sex_id
from vw_cpims_cpara where (current_date - date_of_event) <= 457 {cbos}
)cp
...
where exit_status='ACTIVE' and (current_date - date_of_event) <= 550 {cbos}
...
from vw_cpims_dash_attrition {ocbos} group by services, agency
