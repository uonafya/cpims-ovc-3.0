...
from .charts import (
    ...
    column_stacked_chart, pyramid_chart)
...
def get_chart_data(request, rid, county_id, const_id,
    ...
    try:
        ...
        elif ctt['ctype'] == 'column_stacked':
            resp = column_stacked_chart(request, params, data)
        elif ctt['ctype'] == 'cpyramid':
            resp = pyramid_chart(request, params, data)
        else:
...
def get_raw_data(rid, params):
    ...
    try:
        ...
        data['items'] = items
        for rdt in rdata:
            data[rdt] = ','.join(rdata[rdt])
        itm_list = []
        for itm in items:
            itm_list.append(['%s' % itm, 1])
        data['items_list'] = itm_list
        print(data)
    except Exception as e:
